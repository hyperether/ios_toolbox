...
public extension UIImage {
    ...
    func croped(proportion: CGFloat = 0.5) -> UIImage? {
        let height = self.size.width > self.size.height ? self.size.height : self.size.width * proportion
        let width = self.size.width > self.size.height ? self.size.height * proportion : self.size.width
        let cropRect = CGRect(x: 0, y: (size.height - height) / 2 , width: width, height: height)
    ...
    func isImageEqual(to image: UIImage) -> Bool {
